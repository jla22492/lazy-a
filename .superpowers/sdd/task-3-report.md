...
- Fresh-production-build Real-Chrome hero occlusion/pixel parity: PASS. Upright error `1.887` versus
  ...
  `2.267 / 1.477 / 1.671`; first live hero frame MAD
  `5.838 / 4.494 / 5.887`.
- Fresh-production-build Real-Chrome arrival continuity: PASS at all five
  ...
  pixel drift.
- Fresh-production-build Real-Chrome lifecycle: expected boundary `40/45`. All
  five viewports pass
  observability, pre-settle zero, decoded coverage, profile, four-corner
  registration, authored depth, and `loop=false`; each then fails only while
  loading the intentionally absent catalog (`404/200`).
...
- `encode-master-shots.mjs --verify-only`: expected Task 4 media boundary, 12
  issues. At the final Task 3 observation wide CONTACT had `50/58` source PNGs
  and portrait `28/58`; both forward/reverse encodes remained the old 28-frame
  media and endpoint source relationships were therefore absent.
...
- ✅ Media/lifecycle ownership: one browser-observable hero video, one play
  after settle, navigation-independent continuation, one end, final hold.
