...
# Muntins (0100) so the pano carries the true lattice.
for i, f in enumerate((1/3, 2/3)):
    box("muntinH" + str(i), (0.02, 1.1, 0.028), (2.25, 0.6, 0.9 + 1.1 * f), plaster)
for i, f in enumerate((1/3, 2/3)):
    box("muntinV" + str(i), (0.02, 0.028, 1.1), (2.25, 0.05 + 1.1 * f, 1.45), plaster)
...
cam.rotation_euler = (1.5707963, 0, 0)  # level horizon
scene.camera = cam
...
scene.render.filepath = f"{outdir}/pano-spike.jpg"
